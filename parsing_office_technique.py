...
for product in main_block:
    ...
    product_price = product.find("div", class_="list-view__price").find("span", class_="text-dark h5").get_text()
    # print(product_price)
    product_image = product.find("div", class_="list-view__img mr-4").find("img")["src"]


    data_Base.append({
        "Technique_name": product_name,
        "Technique_price": product_price,
        "Technique_image": product_image
    })